...
public class Utils {
	...
	public static void facilitiesIntoNeo4jSpain(Config config) throws Exception {
		String database = config.getNeo4JConfig().getDatabase();
		try( Neo4jConnection conn = new Neo4jConnection()){  
			conn.query(database,"MATCH (x:OSMNode)-[:TAGS]->(t:OSMTags)"
					+ "  WHERE (exists(t.amenity) OR exists(t.tourism) OR exists(t.natural)) AND NOT (x)-[:ROUTE]->() set x:FacilityNode return count(x)",AccessMode.WRITE);
			conn.query(database,"CREATE INDEX FacilityNodeIndex FOR (n:FacilityNode) ON (n.node_osm_id)",AccessMode.WRITE);
