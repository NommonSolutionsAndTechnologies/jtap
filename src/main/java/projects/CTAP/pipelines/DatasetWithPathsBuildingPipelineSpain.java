...
public class DatasetWithPathsBuildingPipelineSpain implements Callable<Integer> {
	...
	private static final Logger log = LogManager.getLogger(DatasetWithPathsBuildingPipelineSpain.class);

	public static void main(String[] args) {
		System.exit(new CommandLine(new DatasetWithPathsBuildingPipelineSpain()).execute(args));
	}
	...
	public Integer call() throws Exception {
        ...
        String destinationsIDs = config.getCtapModelConfig().getDatasetConfig().getNewDatasetParams().getDestinationsIDs();
		List<Long> citiesDs_ids = Arrays.asList(destinationsIDs.split(",")).stream().map(s -> Long.parseLong(s.trim())).collect(Collectors.toList());
        
		//List<Long> citiesOs_ids = cities.stream().filter(e -> e.getId() == 3L).map(CityNode::getId).collect(Collectors.toList());     //just for test
		//List<Long> citiesOs_ids = cities.stream().filter(e -> e.getPopulation() <= 10000).map(CityNode::getId).collect(Collectors.toList());
		
		List<Long> citiesOs_ids = new ArrayList<>(){
            {
                add(509L);
            }
        };
		...
		List<Long> testActLoc = Arrays.asList(destinationsIDs.split(",")).stream().map(s -> Long.parseLong(s.trim())).collect(Collectors.toList());
