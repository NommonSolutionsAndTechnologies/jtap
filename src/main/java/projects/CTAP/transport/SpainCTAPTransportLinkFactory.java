...
public class SpainCTAPTransportLinkFactory extends AbstractCTAPTransportLinkFactory {
		...
		if (beforeSchOpt) {
			...
			if(inputData.get("to_node_type").equals("Air")){
				...
				//TODO make the cross links from cities to airports dependent on size of city? 
					...
					CrossLinkValue = (long) (((Long) inputData.get("avg_travel_time")).doubleValue() * tlc.getMvTransferTime().getBus() +
							(idealConnectionTimeRoadToAir * tlc.getMvWaitingTime()));
				...
				if(inputData.get("from_node_type").equals("Rail") && (inputData.get("to_node_type").equals("City") || inputData.get("to_node_type").equals("Road"))) {
					// To prevent an agent from changing the mode to go to the airport instead of doing the whole journey by train. The weight is increased by 60. This value must be subtracted in the logit model.
					CrossLinkValue = (long) (((Long) inputData.get("distance")).doubleValue() / 1.42 * tlc.getMvTransferTime().getWalk()) + 60; 
				}
				else {
					CrossLinkValue = (long) (((Long) inputData.get("distance")).doubleValue() / 1.42 * tlc.getMvTransferTime().getWalk());
				}
	...
	public double CTAPTransportRailLinkValue(Object tlc_, Map<String, Object> inputData) {
		...
		return Double.parseDouble(inputData.get("avg_travel_time").toString()) *tlc.getMvTransferTime().getTrain() 
				+ Double.parseDouble(inputData.get("avg_travel_time").toString()) *55.55/1000*tlc.getPriceXKm().getTrain()
				+ reliability
